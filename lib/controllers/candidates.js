...
exports.upload = function(req, res){
  fs.readFile(req.files.resume.path, function (err, data) {
    var vd = "/images/resume" + (Math.random() * 1000000000000) + req.files.resume.name;
    var newPath = __dirname + "/../../public" + vd;
    fs.writeFile(newPath, data, function (err) {
      console.log(err);
      res.send(200, '{"ok": true, "data": "' + vd + '"}');
      //res.send({ok: true, url: vd});
    });
  });
};
  ...
  Candidate.findById(userId).populate('interview.interviewer').exec(function (err, user) {
    ...
    if (user) {
      ...
      Job.find({'applicants.candidate': userId}, function(err, jobs){
        ...
        Event.find({'attendees': userId}, function(err, evt){
          user.events = evt || [];
